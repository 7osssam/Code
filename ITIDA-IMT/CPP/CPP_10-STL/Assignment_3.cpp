...
bool findElement(const std::vector<int>& vec, int value)
{
	auto pos = std::find(std::begin(vec), std::end(vec), value);
	if (pos != std::end(vec))
	{
		return true;
	}
...
void test_findElement()
{
	...
	assert(findElement(vec1, 5) == true);
	assert(findElement(vec1, 4) == false);

	std::cout << "====== Testing findElement() Passed ======" << std::endl;
}
